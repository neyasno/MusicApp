...
func CreateTracksDatabase(db database.Database){
	...
	for i := 0; i < 1000; i++ {
		...
		album := albums.GetRandomAlbum()

		track := database.TrackData{
			Id: primitive.NewObjectID(),
			Title: songs[rand.Intn(100)],
			Filename: filenames[rand.Intn(100)],
			Duration: durations[rand.Intn(100)],
			Genre: genres[rand.Intn(100)],
...
func CreatePlaylistsDatabase(db database.Database){
	...
	for i := 0; i < 50; i++ {
		...
		for j := 0; j < tracksInPlaylist; j++ {
			...
			newTrack := tracks.GetRandomTrackByAuthor()
...
func CreateContentLines(db database.Database){
	...
	contentLinesArray := []database.ContentLine{playlistsLine , authorsLine , albumsLine , tracksLine}
